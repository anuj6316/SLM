...
from core.prompts import format_example
...
def train_model(config: AppConfig):
    ...
    with console.status("[bold yellow]Preparing dataset and applying chat template...") as status:
        ...
        dataset = dataset.map(lambda example: format_example(example, tokenizer, config))
        dataset = dataset.train_test_split(test_size=config.training.test_size, seed=42)
